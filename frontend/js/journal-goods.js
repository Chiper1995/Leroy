function Goods(elementId, treeViewElementId) {
    ...
    this.getCountOfId = function (id) {
        var count = 0;
        $("div#journal-goods :input.goods-id").each(function () {
            if (id === $(this)[0].value)
                count++;
        });
        return count;
    ...
    jQuery(document).ready(function () {
        ...
        // Дублированние товара
        $body.on("click", $this.elementId + " .goods-duplicate", function () {
            var $$this = $(this);

            bootbox.dialog({
                message: "Дублировать этот товар?",
                title: "Подтверждение",
                buttons: {
                    success: {
                        label: "Да",
                        ...
                        callback: function () {
                            ...
                            if (node !== undefined) {
                                ...
                                $($this.treeViewElementId).treeview('selectNode', [node, {silent: true}]);
                                $this.addRow(node.id, node.name);
