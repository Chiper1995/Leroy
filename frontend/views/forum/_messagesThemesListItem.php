...
<div class="col-md-7 column-with-icon">
    <div class="im">
        <a href="<?= Url::to(['forum/messages', 'id'=>$model->id, 'returnUrl'=>Yii::$app->request->url])?>" data-pjax="0">
            <?= Html::tag('div', Html::tag('i', '', ['class' => 'fa fa-comments-o']),['class' => 'img-circle']); ?>
        </a>
    </div>
    <?=Html::a($model->name, ['forum/messages', 'id'=>$model->id, 'returnUrl'=>Yii::$app->request->url])?>
</div>
...
<div class="col-md-3 column-with-buttons<?php if(!$canEdit):?> hidden-sm hidden-xs<?php endif;?>">
    ...
    <?php if($canEdit):?>
        <div class="edit-buttons">
            <?= Html::a(Html::icon('pencil'), ['update-message-theme', 'id'=>$model->id, 'returnUrl'=>Yii::$app->request->url], ['class' => 'btn btn-primary btn-xs']) ?><br>
