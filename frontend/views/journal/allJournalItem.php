    ...
    <div class="caption">
        <?php if ($showAuthor):?>
            <?php if (\Yii::$app->user->can(Rights::SHOW_FAMILY_JOURNAL, ['user'=>$model->user])):?>
                <p>
                    <?= Html::a(Html::icon('user'). Html::encode($model->user->fio), ['journal/family-journal', 'id'=>$model->user->id,], ['class'=>'dark'])?>
                    <?= ( \Yii::$app->user->can(Rights::SHOW_MY_JOURNAL_RECORDS) || \Yii::$app->user->can(Rights::SHOW_MY_JOURNAL_FAVORITES) ) && Yii::$app->user->id !== $model->user_id  ? SubscribeLink::widget($subscribeLinkConfig) : '' ?>
                </p>
            <?php else:?>
