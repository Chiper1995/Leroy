...
?>
    ...
    <div class="col-md-12 container <?= $class ?>" style="padding-bottom: 20px;">
        <div class="feed-filter-form row">
            ...
            */
            ?>
            <?php /*=$form->field($searchModel, 'typeFilter')
...
<?php $this->registerJs(<<<'JS'
    jQuery(document).ready(function () {
        $('#filter-for-feed').on('submit', function(e){
            if($('#useralljournalsmartsearch-smartsearch').val().length>0)
                $(this).append('<input type="hidden" name="UserAllJournalSmartSearch[smartSearch]" value="'+$('#useralljournalsmartsearch-smartsearch').val()+'">');
	  	});
    });
JS
); ?>