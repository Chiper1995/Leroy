...
class Html
{
    ...
    private static function parseText($node, $element, &$styles)
    {
        ...
        if (is_callable(array($element, 'addText'))) {
            $element->addText($node->ownerDocument->saveXML($node), $styles['font'], $styles['paragraph']);
        }
    }
    ...
    private static function parseLink($node, $element, &$styles)
    {
        ...
        if (strpos($target, '#') === 0) {
            return $element->addLink(substr($target, 1), $text, $styles['font'], $styles['paragraph'], true);
        }

        return $element->addLink($target, $text, $styles['font'], $styles['paragraph']);
    }
}
